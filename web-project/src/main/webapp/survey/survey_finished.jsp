...
<%
	...
	int count = surveyDao.getSurveyFinishedCount();
    ...
    list = surveyDao.getFinishedSurveyWithLimit(startRow, pageSize);
...
if (count > 0) {
    ...
    for (int i=0; i<list.size(); i++) {
        ...
        %>
        <tr onclick="location.href='survey_finishedContent.jsp?num=<%=survey.getNum() %>&pageNum=<%=pageNum%>'">  
...
<%
	if (count > 0) {
		...
		if (startPage > pageBlock) {
			%><a href="survey_finishedContent.jsp?pageNum=<%=startPage-1 %><%--&search=<%=search%>--%>">[이전]</a><%
		}
		
		// 1~10
		for (int i=startPage; i<=endPage; i++) {
		    %><a href="survey_finishedContent.jsp?pageNum=<%=i %><%--&search=<%=search%>--%>"><%
		    if (i == pageNum) {
		        %><b>[<%=i %>]</b><%
		    } else {
		        %>[<%=i %>]<%
		...
		// [다음]
		if (endPage < pageCount) {
			%><a href="survey_finishedContent.jsp?pageNum=<%=startPage+pageBlock %><%--&search=<%=search%>--%>">[다음]</a><%
		}
		%>
		</div>
		<%
